			...
			for action in rspec.response.monitoring.action:
				...
				try:
					if action.id == "callback":
						...
						ZabbixHelper.sendVMDiscovery(action.server, action.server.virtual_machines)
				...
				if action.status == "ONGOING":
					...
					ZabbixHelper.sendVMDiscovery(server, action.server.virtual_machines)
					ZabbixHelper.sendVMStatusDiscovery(action.server.virtual_machines)
					actionModel.destroy()
