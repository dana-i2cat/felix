        ...
        for xd_topol in topol_list:
            ...
            for node in node_list:
                ...
                if node.has_key('@'+const.XML_ATTR_TYPE):
                    ...
                    if not node_type == const.TYPE_NODE_SE:
                        logger.warn('attribute <{0} {1}={2}> is invalid value.'\
...
class DBUploader:
    ...
    def __upload_monitoring_data(self,**val_dict):
        ...
        # -d "dbi:mysql:monDataSEDB:localhost" -u "root" -p "rootroot"
    ...
    def cleate_monitoring_data_dict(self,md_xml):
            # get Xml root element in the Dictionary type.
    ...
    def cleate_upload_data_list(self,mon_data_dict):
        try:
            # get topology list
            val_dict_list=list()
                    ...
                    for port in port_list:
                        ...
                        for interface in if_list:
                            if interface not in session:
                                interface = session.query(InterFace).get(interface.id) 
            ...
            self.logger.exception('cleate upload monitoring-data-list error.')
            raise

        return val_dict_list

    def upload_monitoring_data_all(self,val_dict_list):
        try:
