...
def monitoring_sdn_post():
    logger.info('POST monitoring-data({0}). -start-'.format(const.TYPE_MON_SDN))
    try:
        ...
        tpldb_close()
    logger.info('POST monitoring-data({0}). -end-'.format(const.TYPE_MON_SDN))

    return

@route(BASE_URI + '/' + const.TYPE_MON_SDN + '/<topology_type>/', method='GET')
def monitoring_sdn_get(topology_type=""):
    logger.info('GET monitoring-data({0}). -start-'.format(const.TYPE_MON_SDN))
    ...
    try:
        ...
        # close topology database connection.
        tpldb_close()
    logger.info('GET monitoring-data({0}). -end-'.format(const.TYPE_MON_SDN))
