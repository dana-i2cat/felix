...
from module.common.md import search_target_table,post_md,DBHandle,DBUploader
from module.common.xml_util import prettify
...
@route(BASE_URI + '/network_sdn', method='POST')
@route(BASE_URI + '/network_sdn/', method='POST')
def monitoring_sdn_post():
    ...
    try:
        ...
        if not param_string:
            ...
            return HTTPResponse(status=400, body="POST monitoring-data({0}) error.(request body is empty.)".format(const.TYPE_MON_SDN))

        # upload all monitoring-data to DB.
        mon_data_dict = db_upld.upload_monitoring_data_all(param_string)
        if not mon_data_dict:
            return HTTPResponse(status=400, body="POST monitoring-data({0}) error.(parse monitoring-data xml error.)".format(const.TYPE_MON_SDN))
        
        #forwarding master-monitoring-server
        if mon_data_dict[const.XML_ATTR_FWD] == True:
            res_flg,res = post_md(POST_URI,param_string,'no')
            if res_flg is False:
                logger.error('forward monitoring-data error.(post_uri={0})'.format(POST_URI))
            if res:
                logger.debug(res.text)
                
    except Exception:
        logger.exception('POST monitoring-data({0}) error.'.format(const.TYPE_MON_SDN))
        return HTTPResponse(status=500, body="POST monitoring-data({0}) error.".format(const.TYPE_MON_SDN))
...
@route(BASE_URI + '/<monitoring_type>/<topology_type>/', method='GET')
def monitoring_get( monitoring_type="", topology_type=""):
    logger.info('GET monitoring-data({0}).'.format(monitoring_type))
    ...
    try:
        ...
        #monitoring_type=='network_sdn'
        logger.debug('monitoring type=' + monitoring_type)

        #topology_type==[physical | slice]
        logger.debug('topology type=' + topology_type)
        
        #param_type_list==[status | in_bps | out_bps]
        ...
        # required check.(monitoring_type)
        if not monitoring_type == const.TYPE_MON_SDN and \
            not monitoring_type == const.TYPE_MON_TN and \
            not monitoring_type == const.TYPE_MON_CP:

            logger.warn('monitoring type is invalid. ('+ monitoring_type +')')
            return HTTPResponse(status=400, body="GET monitoring-data({0}) error.(monitoring type({1}) is invalid.)"
                                .format(const.TYPE_MON_SDN,monitoring_type))
        ...
        if not topology_type == const.TYPE_NW_PHYSICAL and \
            ...
            return HTTPResponse(status=400, body="GET monitoring-data({0}) error.(topology type({1}) is invalid.)"
                                .format(const.TYPE_MON_SDN,topology_type))
        ...
        if not param_type_list:
            ...
            return HTTPResponse(status=400, body="GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                .format(const.TYPE_MON_SDN,const.HTTP_GET_OPT_TYPE))
        
        if not param_stime:
            logger.warn('Required HTTP GET option({0}) have not been set.'.format(const.HTTP_GET_OPT_STIME))
            return HTTPResponse(status=400, body="GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                .format(const.TYPE_MON_SDN,const.HTTP_GET_OPT_STIME))

        if not param_etime:
            logger.warn('Required HTTP GET option({0}) have not been set.'.format(const.HTTP_GET_OPT_ETIME))
            return HTTPResponse(status=400, body="GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                .format(const.TYPE_MON_SDN,const.HTTP_GET_OPT_ETIME))
        ...
        if not param_topol:
            ...
            if topology_type == const.TYPE_NW_SLICE:
                ...
                return HTTPResponse(status=400, body="GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                    .format(const.TYPE_MON_SDN,const.HTTP_GET_OPT_TOPOL))
        ...
        tbl_name_list = search_target_table(logger,int(param_stime),int(param_etime))
        ...
        logger.exception('GET monitoring-data({0}) error.'.format(monitoring_type))
        return HTTPResponse(status=500, body="GET monitoring-data({0}) error.".format(const.TYPE_MON_SDN))
