...
from module.common.se_md import DBUploader

# constants
BASE_URI = '/' + config.rest_base + '/' + const.REST_MONITORING
POST_URI = config.post_uri + "/" + const.TYPE_MON_SE
...
@route(BASE_URI + '/' + const.TYPE_MON_SE + '/<topology_type>/', method='GET')
def monitoring_se_get(topology_type=""):
    logger.info('GET monitoring-data({0}). -start-'.format(const.TYPE_MON_SE))
    ...
    try:
        ...
        if not topology_type == const.TYPE_NW_PHYSICAL and \
            ...
            return HTTPResponse("GET monitoring-data({0}) error.(topology type({1}) is invalid.)"
                                .format(const.TYPE_MON_SE,topology_type), status=400)
        ...
        if not param_type_list:
            ...
            return HTTPResponse("GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                .format(const.TYPE_MON_SE,const.HTTP_GET_OPT_TYPE), status=400)
        
        if not param_stime:
            logger.warn('Required HTTP GET option({0}) have not been set.'.format(const.HTTP_GET_OPT_STIME))
            return HTTPResponse("GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                .format(const.TYPE_MON_SE,const.HTTP_GET_OPT_STIME), status=400)

        if not param_etime:
            logger.warn('Required HTTP GET option({0}) have not been set.'.format(const.HTTP_GET_OPT_ETIME))
            return HTTPResponse("GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                .format(const.TYPE_MON_SE,const.HTTP_GET_OPT_ETIME), status=400)
        ...
        with util.semaphore_md:
            ...
            if not param_topol:
                ...
                if topology_type == const.TYPE_NW_SLICE:
                    ...
                    return HTTPResponse("GET monitoring-data({0}) error.(Required HTTP GET option({1}) not specified.)"
                                        .format(const.TYPE_MON_SE,const.HTTP_GET_OPT_TOPOL), status=400)
            ...
            tbl_name_list = search_target_table(logger,int(param_stime),int(param_etime),config.mon_data_se_db)
        
            # add <topology_list>
            xml_topol_list = SubElement(xml_mon_data, const.XML_TAG_TOPOL_LIST)
    
            # connect to the database.
            db_con = db_handle.connect(config.mon_data_se_db,config.db_addr
        ...
        logger.exception('GET monitoring-data({0}) error.'.format(const.TYPE_MON_SE))
        ...
        # close topology database connection.
        tpldb_close()     
    logger.info('GET monitoring-data({0}). -end-'.format(const.TYPE_MON_SE))
