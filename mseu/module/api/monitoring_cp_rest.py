...
from module.common.cp_md import DBUploader
...
@route(BASE_URI + '/' + const.TYPE_MON_CP, method='POST')
@route(BASE_URI + '/' + const.TYPE_MON_CP + '/', method='POST')
def monitoring_cp_post():
    logger.info('POST monitoring-data({0}).'.format(const.TYPE_MON_CP))
    try:
...
@route(BASE_URI + '/' + const.TYPE_MON_CP + '/<topology_type>/', method='GET')
