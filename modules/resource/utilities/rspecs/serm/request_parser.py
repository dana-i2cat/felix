...
class SERMv3RequestParser(ParserBase):
    ...
    def __check_se_node_resource(self, node):
    ...
    def __check_se_link_resource(self, link, c_manager):
    ...
    def get_nodes(self, rspec):
        ...
        for n in rspec.findall(".//{%s}node" % (self.none)):
            if not self.__check_se_node_resource(n):
    ...
    def get_links(self, rspec):
        ...
        for l in rspec.findall(".//{%s}link" % (self.none)):
            ...
            if not self.__check_se_link_resource(l, manager_):
