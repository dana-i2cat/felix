...
class RspecException(TnrmException):
    def __init__(self, prog, reason):
        super(RspecException, self).__init__("Rspec has errors in [%s]: %s" % (str(prog), str(reason)))
        self._desc = "Rspec has errors in [%s]: [%s]" % (str(prog), str(reason))
        print "Rspec has errors in [%s]: [%s]" % (str(prog), str(reason))
