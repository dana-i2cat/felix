class AllowedOrganisations:
    ...
    def get_organisations_type_with_alias():
        ...
        flattened_values = [item for sublist in AllowedOrganisations.ALIASES.values() for item in sublist]
    ...
    def get_organisations():
        ...
        for key in AllowedOrganisations.get_organisations_key():
            if key in organisations:
                organisations[key].extend(AllowedOrganisations.__dict__.get(key))
            else:
    ...
    def get_organisations_with_alias():
        ...
        for key in AllowedOrganisations.ALIASES.keys():
            org_for_alias = AllowedOrganisations.find_organisation_by_alias(key)
            value = AllowedOrganisations.ALIASES.get(key)
            # If the aliases are not already included, do so
            if org_for_alias in organisations and len(set(value).intersection(set(organisations[org_for_alias]))) == 0:
