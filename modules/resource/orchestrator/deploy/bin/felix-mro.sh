...
FELIX_MRO_NAME=felix-mro

# Title for the service, used in service commands
FELIX_MRO_TITLE="FELIX Master Resource Orchestrator"

# Name of the user to be used to execute the service
FELIX_MRO_USER=root
#FELIX_MRO_USER=i2cat

# In which directory is the shell script that this service will execute
#FELIX_MRO_HOME=/opt/felix/felix-dev/modules/resource/orchestrator
FELIX_MRO_HOME=/opt/felix/resource-orchestrator-mro/modules/resource/orchestrator
...
PID_FILE=/var/run/$FELIX_MRO_NAME.pid

# File to handle instances of the program
LOCK_FILE=/var/lock/$FELIX_MRO_NAME

# Where to write the spreader log file
LOG_FOLDER=$FELIX_MRO_HOME/log
...
EXEC="$PROCESS_TYPE $FELIX_MRO_HOME/src/main.py"

# Return variables
RET_SUCCESS=0
RET_FAILURE=1

export FELIX_MRO_HOME
...
create_log() {
	...
	if [ ! -L /var/log/$FELIX_MRO_NAME ]; then
		ln -sf $LOG_FOLDER/ /var/log/$FELIX_MRO_NAME
	fi
}
...
do_start() {
	if [ "$PID" != "" ]; then
		...
		if [ -f "/proc/$PID" ]]; then
			...
			if [ "$?" = "0" ]; then
				...
				echo "$FELIX_MRO_TITLE is already running." 
				rm -f $LOCK_FILE
			fi
		else
	...
	if [ -e /etc/redhat-release ]; then
#		daemon $EXEC --pidfile $PID_FILE --user $FELIX_MRO_USER start > \
#		/dev/null 2> /dev/null &
#		PID=`ps xaww | grep "$PROCESS_TYPE" | grep "$EXEC" | grep "pidfile $PID_FILE" | tail -1 | awk '{print $1}'`
		...
		#start-stop-daemon --start --chuid $FELIX_MRO_USER --user $FELIX_MRO_USER \
		#--name $FELIX_MRO_USER -b --make-pidfile --pidfile $PID_FILE --exec $EXEC
...
do_stop() {
#	daemon $EXEC --pidfile $PID_FILE --user $FELIX_MRO_USER stop > /dev/null 2> /dev/null &
...
#		kill $PID
		for i in {1..30}
		do
			if [ -n "`ps aux | grep $PROCESS_TYPE | grep $FELIX_MRO_NAME `" ]; then
		...
		echo "$FELIX_MRO_TITLE is already NOT running."
		return $RET_SUCCESS
	fi
...
do_check_status() {
	...
	echo "$FELIX_MRO_TITLE is $STATUS."
}

case "$1" in
	start)
		action="Starting $FELIX_MRO_TITLE.";
		echo $action;
		do_start;
		exit $?
	;;
	stop)
		action="Stopping $FELIX_MRO_TITLE.";
		echo $action;
		do_stop;
		exit $?
	;;
	restart|force-reload)
		action="Restarting $FELIX_MRO_TITLE.";
		echo $action;
		do_restart;
		exit $?
	;;
	...
	;;
	*)
		echo "Usage: service $FELIX_MRO_NAME {start|stop|restart|force-reload|config|status}";
		exit $RET_SUCCESS
	;;
esac
