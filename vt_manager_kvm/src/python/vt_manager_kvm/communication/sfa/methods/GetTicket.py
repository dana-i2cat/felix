...
from vt_manager_kvm.communication.sfa.util.vt_manager_kvm.communication.sfa.ablesRuntime import run_vt_manager_kvm.communication.sfa.ables
...
class GetTicket(Method):
    ...
    def call(self, xrn, creds, rspec, users, options):
        ...
        # filter rspec through vt_manager_kvm.communication.sfa.ables
        if self.api.interface in ['aggregate']:
            chain_name = 'OUTGOING'
        elif self.api.interface in ['slicemgr']:
            chain_name = 'FORWARD-OUTGOING'
        rspec = run_vt_manager_kvm.communication.sfa.ables(chain_name, hrn, origin_hrn, rspec)
