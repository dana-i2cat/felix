from vt_manager_kvm.communication.sfa.util.xrn import Xrn, hrn_to_urn, urn_to_hrn
from vt_manager_kvm.communication.sfa.util.sfatime import utcparse, datetime_to_string

from vt_manager_kvm.communication.sfa.sfa_config import config
...
from vt_manager_kvm.communication.sfa.rspecs.elements.ocf_vt_server import OcfVtServer
from vt_manager_kvm.communication.sfa.rspecs.version_manager import VersionManager

from vt_manager_kvm.communication.sfa.rspecs.elements.range import Range
from vt_manager_kvm.communication.sfa.rspecs.elements.network_interface import NetworkInterface
from vt_manager_kvm.communication.sfa.rspecs.elements.vm import VM
from vt_manager_kvm.communication.sfa.rspecs.elements.vm_interface import VMInterface

from vt_manager_kvm.communication.sfa.drivers.VTShell import VTShell
		    ...
		    if slices:
			    ...
			    if vm['node-name'] == node.name:
		    	    	slivers.append(VM({'name':vm['vm-name'],
                                ...
                                services.append(Login({'authentication':"ssh-keys", 
                                                         'hostname':vm['vm-ip'],
                                                         'port': "22",
                                                         'username':'root',}))
                ...
                for node in nodes:
                    ...
                    if slices:
                        for vm in slices['vms']:
                            ...
                            if vm['node-name'] == node.name:
                                single_node['slivers'] = [(VM({'name':vm['vm-name'],
                                                   'state':vm['vm-state'],
                                                   'ip':vm['vm-ip'],
                                                  }))]
                                single_node['services'] = [Login({'authentication':"ssh-keys",
                                                         'hostname':vm['vm-ip'],
                                                         'port': "22",
                                                         'username':'root',})]
